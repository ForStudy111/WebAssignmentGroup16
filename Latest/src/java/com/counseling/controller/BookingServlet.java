...
public class BookingServlet extends HttpServlet {
    ...
    private final GoogleCalendarConnectionDAO googleCalendarConnectionDAO = new GoogleCalendarConnectionDAO();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (action) {
            ...
            // Old cancellation links are sent to the newer cancellation servlet.
            case "cancel":
            case "counsellorCancel":
            case "adminCancel":
                redirectLegacyCancelRoute(request, response, currentUser);
                break;
    ...
    private void approveBooking(HttpServletRequest request,
        ...
        if (student == null) {
            bookingDAO.updateGoogleCalendarInfo(bookingId, null, null, "FAILED");
            redirectWithMessage(request, response,
                    "/BookingServlet?action=counsellorList",
        ...
        if (!googleCalendarConnectionDAO.isConnected(currentUser.getUserId())) {
            bookingDAO.updateGoogleCalendarInfo(
                    bookingId, null, null, "NOT_CONNECTED"
            );
            redirectWithMessage(request, response,
                    "/BookingServlet?action=counsellorList",
        ...
        try {
            String refreshToken = googleCalendarConnectionDAO.getRefreshToken(
                    currentUser.getUserId()
            );
            ...
            boolean calendarSaved = bookingDAO.updateGoogleCalendarInfo(
                    bookingId,
    ...
    private void redirectLegacyCancelRoute(HttpServletRequest request,
            HttpServletResponse response,
            User currentUser) throws IOException {
    ...
    private void prepareBookingViewData(HttpServletRequest request,
        ...
        for (Booking booking : bookingList) {
            ...
            if (schedule != null) {
                ...
                if (!counsellorMap.containsKey(schedule.getCounsellorId())) {
                    ...
                    if (counsellor != null) {
                        counsellorMap.put(
                                counsellor.getCounsellorId(), counsellor
                        );
                    }
                }
            }
